...
**Feature pipeline:** `/feature-next` → (`/brainstorm` or `/feature-new` or `/feature-ingest <#>`) → `/feature-triage` → `/feature-research` → `/feature-plan` → `/feature-plan-review` → `/feature-plan-handoff` → `/feature-implement` → `/review-loop` → `/merge-gate`. Triage's `complexity:` routes the depth: `S` runs the fast lane (main-thread research, inline plan approval, second-opinion subagent only for ≥3-file or user-visible plans); `M`/`L` run the full lane (Explore agents, second opinion, HTML plan review). `/brainstorm` is the pre-pipeline problem-space stage: given a vague idea it interrogates the problem (never the implementation), drafts the spec's `## Problem` / `## Desired behavior` / `## Success criteria` / `## Non-goals`, and hands them to `/feature-new` to create the issue and the spec. Reach for it when the idea is not yet a concrete observable change; `/feature-loop` refuses such a description and points here.

Canonical lifecycle: `TRIAGE → RESEARCH → PLAN → IMPLEMENT → REVIEW → GATE → DONE`. `REVIEW` = PR open, `/review-loop` driving convergence (writes a per-iteration line to the plan's `## PR convergence ledger`). `GATE` = review converged, `/merge-gate` validating the **still-open** PR against the spec's `## Success criteria` (writes `## Gate verdict`). `DONE` = gate PASS recorded and the plan moved to `docs/exec-plans/completed/`. A plan declaring a non-final `Phase: N of M` (`N < M`) is the exception: the gate records a per-phase PASS and holds the spec at `GATE` instead. The merge is a separate later step, so a spec can be `DONE` while its PR is still open. Each stage skill reads `stage:` from the spec's frontmatter and refuses if mismatched, so any skill can be run cold from a fresh agent context.
...
**Inbound PR queue:** `/pr-queue` is the maintainer-side counterpart to the feature pipeline — it orders the open PRs, triages each read-only (premise check first), explains each in user-visible terms, and gates every action on a per-PR decision. Execution routes on push-ability: a fork or protected branch gets `/review-pr` read-only, a pushable branch may get `/review-loop`. Rationale in `docs/design-docs/pr-queue.md`.
...
**Pre-merge gate:** `/merge-gate` checks the open PR against the spec's `## Success criteria` and `## Non-goals` plus doc accuracy. It does **not** re-run build/lint/test — `/feature-implement` runs those before the commit and CI runs them on every push. PASS advances Stage → DONE, moves the plan to `completed/`, and commits that bookkeeping to the **feature branch**, so a feature ships in one PR instead of a follow-up chore PR. A plan declaring a non-final `Phase: N of M` (`N < M`) is the exception: the gate records a per-phase PASS and holds the spec at `GATE` instead. FAIL holds at GATE and the fix goes into the same PR; it files no follow-up issues, because nothing has shipped yet.
...
- **Installer upgrade check:** `tests/install-upgrade-check-test.sh` (opt-out key, legacy cron/flag removal, `--status`/`--doctor`, uninstall cleanup, post-pull re-exec). Runs real installs in a scratch `HOME` with a recording `crontab` stub.
- **Entry-skill preamble sync:** `scripts/upgrade/sync-preamble.sh --check` (also run by `scripts/upgrade/check-test.sh`). Edit `scripts/upgrade/preamble.md`, never the block inside a `SKILL.md`, then run `scripts/upgrade/sync-preamble.sh`.
- **Install smoke:** `HOME=$(mktemp -d) && mkdir -p "$HOME/.claude" && ./install.sh --prefix hs- --dry-run` (then repeat with `--prefix ""`).
- **Render correctness:** `HOME=$(mktemp -d) && mkdir -p "$HOME/.claude" && ./install.sh --prefix hs-` then `grep -q '/hs-feature-plan' .rendered/hs-/skills/hs-feature-research/SKILL.md` and `! grep -q '/feature-plan\b' .rendered/hs-/skills/hs-feature-research/SKILL.md`.
