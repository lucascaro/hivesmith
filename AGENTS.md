...
**Feature pipeline:** `/feature-next` → (`/feature-new` or `/feature-ingest <#>`) → `/feature-triage` → `/feature-research` → `/feature-plan` → `/feature-plan-review` → `/feature-plan-handoff` → `/feature-implement` → `/review-loop` → `/merge-gate`. Triage's `complexity:` routes the depth: `S` runs the fast lane (main-thread research, inline plan approval, second-opinion subagent only for ≥3-file or user-visible plans); `M`/`L` run the full lane (Explore agents, second opinion, HTML plan review).
