...
**Open for operator decision (raised in review iteration 3, findings #9/#10):** the committed `.claude/settings.json` is a trust boundary. Two consequences that the original "commit the hook config" decision did not weigh: (a) the PreToolUse entries invoke a bare `graphify hook-guard`, so anyone cloning hivesmith without graphify installed gets exit 127 on every `Bash`/`Grep`/`Read`/`Glob` call, and nothing documents graphify as a contributor prerequisite; (b) the PostToolUse hook targets `scripts/graphify-refresh.sh`, a repo-tracked path — a contributor PR editing that script's body gets code execution on any maintainer who checks the branch out and makes one edit, because what gets surfaced for review is the unchanging command string, not the file it points at. Options: point the hook at an untracked path written by setup (e.g. under `.git/`), stop committing the hooks and keep `/graphify-init` purely opt-in, or document the prerequisite and accept it.
