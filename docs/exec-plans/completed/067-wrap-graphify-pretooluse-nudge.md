...
- **2026-09-04** — Mirror upstream's `.strip().lower()` normalisation for `GRAPHIFY_HOOK_STRICT` rather than matching a fixed set of spellings. Why: any spelling the two disagree on is a silently swallowed deny, and the asymmetry favours the wrong direction — graphify thinks it is blocking, the wrapper never asks it.
...
- **2026-09-04** — Review iteration 1: COMMENT, zero threads (converged). Fixed the one IMPORTANT finding rather than deferring it — `strict_enabled` matched a narrower set of spellings than upstream's `.strip().lower()`, so `GRAPHIFY_HOOK_STRICT=True` / `Yes` / `" 1"` was strict-ON for graphify and strict-OFF for the wrapper: the pre-fork shortcut fired and a strict-mode deny was silently swallowed. That is the single decision the wrapper promises to leave entirely to graphify, so a silent drop is worse than a wrong nudge. Verified by negative control — restoring the narrow matcher fails the suite on exactly those three spellings. Also took four minors: session-id extraction now uses a bash regex (leftmost-first, so genuinely the FIRST occurrence the comment claimed) with parameter-expansion sanitisation, and stdin is read with a builtin, making the post-claim fast path fork-free; a float `GRAPHIFY_HOOK_STRICT_TTL` is truncated rather than rejected, since upstream parses it with `float()`; the deny pass-through comment no longer overstates itself as byte-for-byte (`$(...)` drops the trailing newline; the JSON is unchanged); and the unwritable-cache sub-case is skipped as root, where `chmod` asserts nothing.
- **2026-09-04** — Second CI run: the production fix held (the stamp case passes on Linux) but `test_nudge_survives_gnu_stat`'s own shim was macOS-only — it deferred to `/usr/bin/stat -f %m` for its answer, which is filesystem mode on Linux. Rewrote the shim to model GNU coreutils without deferring to any real `stat`, so it behaves identically on both platforms. Verified with a negative control: patching the wrapper back to a BSD-first probe makes it emit despite a fresh stamp, while the shipped order stays silent — the test catches the regression it guards rather than passing vacuously.
