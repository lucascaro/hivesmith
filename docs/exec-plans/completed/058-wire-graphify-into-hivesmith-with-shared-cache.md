...
- `skills/graphify-init/test/run-all.sh` — assert-style suite, bash 3.2 compatible, mirroring `scripts/brain/test/run-all.sh`. 19 tests after review iteration 3.
...
- **2026-08-30** (review iter 3) — `mtime_of` probes GNU `stat -c` BEFORE BSD `stat -f`, and rejects non-numeric output. Why: on GNU coreutils `-f` means `--file-system`, so a BSD-first probe printed a filesystem block to stdout AND exited 1; the `||` fallback appended the real mtime to that garbage and the arithmetic then killed the script under `set -u` — before it stamped or launched anything. Every Linux host silently stopped refreshing after the first run. Confirmed verbatim in this PR's own CI (`line 51: File: unbound variable`). BLOCKING; guarded now by `test_refresh_survives_gnu_stat`, which shims a GNU-like stat.
- **2026-08-30** (review iter 3) — Nudge-install failures print to stderr (bypassing `say`, which `--quiet` silences) and a failed DEFAULT-ON install is now fatal. Why: the failure was swallowed into `say` under `--quiet`, so setup exited 0 reporting success with the hooks never registered. `--no-nudges` is the supported way to proceed without them, so silence was never the right degradation. The interpreter resolver was also hardened to try several candidates including the `#!/usr/bin/env python` shebang form, since a pipx/uv-tool venv is invisible to any ambient python.
- **2026-08-30** (review iter 3) — `HOOKS_DIR` derives from `git rev-parse --git-path hooks`, and a hook missing after a successful `graphify hook install` is fatal. Why: hardcoding `$COMMON/hooks` ignored `core.hooksPath` (Husky, lefthook, a global `~/.githooks`) which graphify itself honors — `patch_worktree_guard` found no file, returned 0, and we still printed "worktree guard lifted". Exactly the silent staleness the loud drift check exists to prevent.
- **2026-08-30** (review iter 3) — Test suite hardened: `GIT_CONFIG_GLOBAL=/dev/null` so fixtures cannot inherit a global `core.hooksPath` and write hooks outside the sandbox; the debounce, copy-in-sync, and guard-drift tests now assert exit status rather than passing vacuously. Why: the debounce test repeating the broken `stat` idiom is precisely why the BLOCKING Linux bug shipped green.
- **2026-08-30** (review iter 3) — Install guidance URL corrected to `github.com/Graphify-Labs/graphify` in both SKILL.md files. Why: `github.com/lucascaro/graphify` returns 404 (verified), and PyPI `graphifyy` 0.9.53 declares Graphify-Labs as Homepage/Repository. The wrong URL was pre-existing in `hivesmith-init` and inherited.
- **2026-08-30** (review iter 3) — `ci.yml` declares file-level `permissions: contents: read`. Why: the new job installs an unpinned package from PyPI and inherited the write-capable default token. Orthogonal to the accepted unpinned decision and free. The one job that needs write (`regenerate-generated`) lives in `changesets.yml` with its own job-level `contents: write`, so it is unaffected.
- **2026-08-30** (review iter 3) — SKILL.md now tells the agent to invoke `graphify-setup.sh` from the skill's own directory. Why: an installed skill lives at `~/.claude/skills/<prefix>graphify-init/`, so the repo-relative path only resolved inside a hivesmith checkout — the one project that least needs this skill.
- **2026-08-30** (review iter 3, operator decision) — The committed `PostToolUse` hook targets `$OUT/graphify-refresh.sh` in the **gitignored** output dir, not `scripts/graphify-refresh.sh`. Why: a committed hook aimed at a tracked path is a branch-controlled payload — a contributor PR editing that script's body executes on any maintainer who checks the branch out and makes one edit, while the reviewed command string never changes. Gitignored means a PR cannot change what runs. `setup` now also *ensures* the output dir is ignored (`ensure_out_ignored`), since the whole property rests on that; a test asserts it, and that test is what caught the gap. The tracked copy and its sync test are gone.
- **2026-08-30** (review iter 3, operator decision) — Every committed hook command degrades to a no-op: the refresh hook is `[ -x … ] || exit 0`, and graphify's own nudges are rewritten post-install to `command -v graphify >/dev/null 2>&1 && … || exit 0`. Why: a bare `graphify hook-guard` in a committed settings.json means exit 127 on every `Bash`/`Grep`/`Read`/`Glob` for anyone who clones without graphify installed.
