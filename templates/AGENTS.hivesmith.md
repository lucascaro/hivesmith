...
**Issue-creation policy:** `.hivesmith/config.toml` sets `[github] create_issues` to one of: `opt-out` (create on GitHub by default, confirm at Gate 1 — recommended), `always` (create without asking; Gate 1 is skipped), `opt-in` (keep specs local by default; only create when asked), or `ask` (no default; prompt every time). `/feature-new` honors this at its Gate 1 — the recommended option flips based on the policy, the user can override, and `always` skips the gate entirely. `/feature-loop` resolves the policy without prompting (`opt-out`/`always` create, `opt-in` stays local) and prompts only under `ask`. Default when the file is missing: `opt-out`.
