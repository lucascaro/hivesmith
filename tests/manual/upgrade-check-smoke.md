...
10. **Background fetch survives the tool call.** `rm ~/.hivesmith/upgrade-check/last-fetch`, note `stat -f %m "$SB/hivesmith/.git/FETCH_HEAD"`, run `/hs-feature-next`. The Bash call for the check returns immediately; a few seconds later `FETCH_HEAD`'s mtime has advanced. Repeat in pi and codex if you use them — a harness that kills the tool's process group would leave `FETCH_HEAD` unchanged, which silently disables the "behind" signal there.
