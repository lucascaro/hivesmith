#!/usr/bin/env bash
# hivesmith telemetry: record a SubagentStop event.
#
...
#
# A SubagentStop with no agent_type is recorded with agent_type: null plus the payload's
...
if command -v jq >/dev/null 2>&1; then
    printf '%s' "$INPUT" | jq -c \
        --arg ts "$TS" --arg ev "subagent_stop" --arg proj "$PROJECT" '{
    ...
    printf '{"ts":"%s","event":"%s","project":"%s","agent_id":"%s","agent_type":"%s","jq":false}\n' \
        "$TS" "subagent_stop" "$PROJECT" "${AI:-}" "${AT:-}" >> "$LOG" 2>/dev/null
fi
