...
5. **Copy selected files** from `<hivesmith>/templates/` into the project, creating directories as needed:
   ...
   - `templates/scripts/check-changeset.sh` + `templates/scripts/check-changeset-selftest.sh` → `scripts/` (chmod +x), and `templates/scripts/hooks/pre-push` → `scripts/hooks/pre-push` (chmod +x). Required by the changeset steps in the scaffolded `changesets.yml` — copy them whenever that workflow is copied. Tell the user to tune the `EXEMPT` list at the top of `check-changeset.sh` (e.g. drop `^scripts/` if their scripts ship to users) and to install the hook with `cp scripts/hooks/pre-push "$(git rev-parse --git-common-dir)/hooks/pre-push"`.
