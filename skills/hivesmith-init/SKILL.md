...
5. **Copy selected files** from `<hivesmith>/templates/` into the project, creating directories as needed:
   ...
   - `templates/scripts/check-changeset.sh` + `templates/scripts/check-changeset-selftest.sh` → `scripts/` (chmod +x), and `templates/scripts/hooks/pre-push` → `scripts/hooks/pre-push` (chmod +x). Skip any that already exist unless `--force`. Required by the changeset steps in the scaffolded `changesets.yml` — copy them whenever that workflow is copied. Then:
     - **Tune the exempt list together with its self-test.** `EXEMPT` at the top of `check-changeset.sh` lists paths that never need a changeset (docs, planning, root markdown, `.github/`, `scripts/`, tests). If the project's scripts ship to users, drop `^scripts/` **and** the matching `scripts/ is exempt` case at the bottom of `check-changeset-selftest.sh`; CI runs the self-test before the gate, so a list edited without its cases fails every PR.
     - **Install the hook without clobbering one.** Resolve `hooks="$(git rev-parse --git-path hooks)"` (honours `core.hooksPath`, e.g. Husky/lefthook). If `$hooks/pre-push` does not exist, copy `scripts/hooks/pre-push` there and chmod +x. If it exists, do not overwrite it: tell the user to add `bash scripts/check-changeset.sh` to their existing pre-push hook (or their hook manager's config).
