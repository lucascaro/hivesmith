...
7. **Apply the changes to the plan.** This skill edits the plan, it does not just complain about it.
   ...
   - **Backfill missing schema sections.** Treat a missing `## Verification` as a grounding finding and fix it in place. **Backfill rule (shared).** A plan scaffolded from an older `docs/exec-plans/_template.md` has no `## Verification`. Both `/feature-plan` and `/feature-plan-review` backfill it — whichever reaches the plan first — by inserting the heading after `### Tests` with real runnable commands. Neither defers to the other; an old plan may legitimately be picked up by either. `/feature-plan-handoff` never backfills: it refuses, so the omission is fixed where it can be reviewed. Likewise fill the spec's `## Non-goals` if the plan implies a boundary the spec never stated.
