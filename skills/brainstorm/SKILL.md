...
4. **Interrogate the problem.** At most **3 rounds**, at most **4 questions per round**, batched — never one question at a time. Use a structured question primitive if the runtime has one (e.g. `AskUserQuestion`), presenting real alternatives with a stated recommendation.
   ...
   **Stop rule.** Stop asking when no remaining unknown would change the problem statement, a success criterion, or a non-goal. Reaching the round limit with something still open is also a stop — record it as an explicit open question rather than asking a fourth round, and carry it to `/feature-new` at step 8 for the spec's `## Notes` (you do not write the spec, so an open question you keep only in this thread is lost).
...
7. **[The gate]** Present the title and all four sections, and ask for approval in the same turn — one question, covering both the content and the GitHub choice, so `/feature-new` does not have to ask again. Read `.hivesmith/config.toml`'s `[github] create_issues` to pick the recommended option (`opt-out` → create; `always` → create; `opt-in` → skip; `ask` → no recommendation; missing → `opt-out`). The policy has four values and all four must map, or an `always` project falls through with no recommendation at the only gate in the flow. Under `always` the gate still presents the sections — it is the content gate, not the GitHub gate — and option 1 is the recommendation:
